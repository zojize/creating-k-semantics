...
- **The builtin `Int`/`Float`/`String` literal tokens silently differ from your language's lexis, and they win the scanner even when you don't want them.** K's `Float`/`Int` token allows a leading sign (`[+\-]?…`), so with no whitespace `a+1` lexes as `a` then the *number* `+1` — a syntax error invisible until a no-space arithmetic vector hits it (a spaced `a + 1` parses fine, so *put un-spaced operators in your suite*). K's `String` token disallows raw newlines and interprets C escapes, unlike e.g. a language whose strings span newlines and have no escapes. The fix is the same for both: **define your own `[token]` for the literal and keep K's builtin one out of the *program* grammar** — do not `imports FLOAT-SYNTAX`/`STRING-SYNTAX` in your `--syntax-module`; import the operations module (`FLOAT`/`STRING`) only in the *semantics* module, and convert the custom token to the builtin value at runtime (`String2Float`, or the `STRING.token2string` hook then strip the quotes). A custom *string* token is not the hazard older advice implied — it is often *required*; the hazard is only letting two literal tokens for the same sort coexist in one lexer.
...
- **Two failure signals worth recognizing on sight.** A *parse* ambiguity the GLR parser can't settle surfaces not at kompile time but at *run* time, as a stuck `amb(...)` term the LLVM backend aborts on (`No tag found for symbol Lblamb{Sort}`) — fix it in the grammar with `[prefer]`/`[avoid]` on the competing production (e.g. `[avoid]` on `"if" "(" Exp ")" Stmt "else" Stmt` makes a dangling `else` bind the nearest `if`). And when a rule you wrote seems never to fire, `grep` the compiled `allRules.txt` for its symbol: a rewrite K silently declined to lower (usually a bad rewrite position or a sort it couldn't place) is simply *absent*, which reads at runtime as a stuck term with "no rules."
...
**Capture the env by *reference*, not by value — or mutual recursion and forward references break.** The captured `Rho` is `name → location`, and the `<store>` is shared, so a binding the closure references can be *defined or updated after* the closure is built and the closure still sees it. That indirection is exactly what makes `f` calling a later-defined `g` (mutual recursion at the same scope), and any forward reference to a binding filled in later, resolve correctly — they are among the *first* conformance tests to fail if you shortcut to a flat `name → value` snapshot (the "monolithic state Map" anti-pattern), which gets simple scope right and then silently diverges. The reference impl's own data structures tell you which: mirror jlox's mutable `Environment` chain / CPython's namespace dicts, not a frozen copy.
...
**Static *name resolution* is the same shape — a second pass over the grammar, separate from execution.** A dynamic env chain answers "what does this name mean?" at use time; a language with *static* binding answers it ahead of time and the runtime must agree. JavaScript hoisting and the TDZ, Python's `global`/`nonlocal` and LEGB, and the *Crafting Interpreters* resolver (which fixes the closure-over-a-later-declared-variable case) are all this: a walk that annotates each reference with a fixed lexical depth (or flags a static error like use-before-declaration), which the interpreter then obeys. Model it like the type checker — its own pass over the same AST — not by contorting the runtime env. Skipping it is a legitimate layer boundary, but it is a real divergence: a purely dynamic chain gets forward-reference scoping subtly wrong, so if you defer the resolver, **say which tests that owes** rather than calling the scope model complete.
...
- **Leaning on the `Exps ::= Vals` subsort + `seqstrict` for n-ary call arguments.** It is the tutorial pattern and often works, but because `Val < Exp` it makes `f()` and `f(true)` parse-ambiguous, and the empty `.Exps` is not always seen as a finished `Vals`, so calls hang on an un-cooled argument list. When it misbehaves, drop the subsort and evaluate the arguments explicitly with a left-to-right accumulator (`evalArgs(callee, pending, done) → invoke(callee, done)`) — duller, but it cannot get stuck.
